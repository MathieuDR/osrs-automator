...
namespace DiscordBot.Modules {
    ...
    public abstract class BaseWaitMessageEmbeddedResponseModule : BaseEmbeddedResponseModule {
        ...
        protected override void BeforeExecute(CommandInfo command) {
            ...
            var user = Context.Guild?.Users.SingleOrDefault(x => x.Id == Context.User.Id);
            if (user != null) {
            ...
            return _waitMessageTask.ContinueWith(waitMessageTask => {
                ...
                if (timeout.TotalSeconds > 1) {
                    ((Task) Task.Delay(timeout)
                        .ContinueWith(_ => _waitMessageTask.Result.DeleteAsync().ConfigureAwait(false))).ConfigureAwait(false);
                } else {
